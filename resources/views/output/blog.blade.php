    ...
    @foreach($posts as $post)
        <div class="mt-6 group relative w-full max-w-xl">
            <ul class="absolute left-full top-2 space-y-1">
                <li>
                    <a href="{{ route('blog.edit', $post)}}" title="Edit link" class="flex items-center justify-center h-10 w-10 bg-gray-700 text-gray-100 hover:bg-cyan-500 hover:text-white" data-turbo-stream>
                ...
                <li>
                    <form action="{{ route('blog.unlink', $post) }}" method="DELETE">
                        ...
                        <button type="submit" data-form-target="submit" title="Unlink" class="flex items-center justify-center h-10 w-10 bg-gray-700 text-gray-100 hover:bg-rose-500 hover:text-white">
            ...
            <div class="bg-white text-gray-600 shadow p-6 space-y-3">
                ...
                <div>
                    ...
                    <p class="text-gray-400 text-xs">Published {{ $post->published_at?->longRelativeDiffForHumans() }} on {{ $post->site }}</p>
                </div>
                <p class="text-gray-700">{{ $post->excerpt }}</p>
                <ul class="text-xs w-full list-none flex flex-wrap items-center text-gray-500">
                    @foreach($post->tags as $tag)
                        <li class="inline-flex items-center mr-2 mb-2">
                            <span class="inline-flex items-center h-7 px-2 bg-gray-200">{{ $tag }}</span>
                        </li>
                    @endforeach
                </ul>
            </div>
